...
public Action OnPlayerRunCmd(int client, int &buttons)
{
	...
	if (g_bToCloseMenu[client] && IsPlayerAlive(client))
	{
		if (buttons & (IN_ATTACK|IN_JUMP|IN_DUCK|IN_FORWARD|IN_BACK|IN_MOVELEFT|IN_MOVERIGHT|IN_ATTACK2))
		{
			...
			CPrintToChat(client, "[{green}SM{default}] Auto-closed the map rating menu to prevent accidental rate. Type {lightgreen}!maprate {default}to reopen it.");
		}
	}
}
...
public Action:Command_Rate(client, args)
{
	...
	if (!g_bCanRateMap)
	{
		CReplyToCommand(client, "[{green}SM{default}] This map is not on the official map rotation therefore you cannot rate it!");
		return Plugin_Handled;
	}
