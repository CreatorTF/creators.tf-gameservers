...
ConVar cvTeamScramble, cvPayloadCart;
...
public void OnPluginStart()
{
	...
	cvPayloadCart = CreateConVar("uv_payloadcart", "1", "Enable/Disable the 'Defend our cart!' or 'Get to the cart!' announcer voicelines.");
...
public void OnRoundActive(Event event, const char[] name, bool dontBroadcast)
{
	...
	switch (g_Gamemode)
	{
		...
		case TF2_GameMode_5CP, TF2_GameMode_KOTH, TF2_GameMode_CTF:
		{
			if (!g_bFight)
			{
				g_bFight = true; //only say this sound for the first round.
				PlaySound("fight");
				playedSound = true;
			}
		}
...
public void OnBroadcastAudio(Event event, const char[] name, bool dontBroadcast)
{
	...
	} else if(TF2_IsHolidayActive(TFHoliday_Halloween))
	{
		if (strcmp(sound, "Game.YourTeamLost") == 0 || strcmp(sound, "Game.Stalemate") == 0)
		{
			event.BroadcastDisabled = true;
			PlaySound("yourTeamLostHalloween", team);
		}
		if (strcmp(sound, "Game.YourTeamWon") == 0)
		{
			event.BroadcastDisabled = true;
			PlaySound("yourTeamWonHalloween", team);
		}
	}
}
...
void PlaySound(const char[] sound, int team = -1) //-1 = all, 2 = red, 3 = blu
{
	...
	if (strcmp(sound, "yourTeamWonHalloween") == 0) // game over to the winning team
	{
		Format(path, sizeof(path), "#creators/music/team_won_halloween.wav");
	}

	if (strcmp(sound, "yourTeamLostHalloween") == 0) // game over to the losing team
	{
		Format(path, sizeof(path), "#creators/music/team_lost_halloween.wav");
	}
