...
public Action OnClientSpray(const char[] te_name, const int[] clients, int client_count, float delay)
{
	...
	if (client && IsClientInGame(client))
	{
		...
		if (!g_bUnderstood[client])
		{
			...
			panel.DrawText(" ");
			panel.DrawText("Pornographic and real life gore, death, \nor abuse content (eg. sprays) is prohibited.");
			panel.DrawText(" ");
			panel.DrawText("By selecting 'I agree' below, I understand that I will \nbe banned if I spray prohibited content listed above.");
			panel.DrawText(" ");
	...
	if (action == MenuAction_Select)
	{
		...
		switch (param2)
		{
			case 1:
			{
				...
				PrintToChat(client, "[SM] Thank you for agreeing to our decal rule. You may now spray decals.");
			}
			case 2:
			{
				PrintToChat(client, "[SM] You may not spray decals till you select 'I agree'. If you truly do not understand the decal rule, send a message in #help-desk on our discord: https://creators.tf/discord");
			}
		}
	}
}